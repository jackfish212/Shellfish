...
AFS has three layers: the VirtualOS orchestrator, the Shell interface, and the Provider backends. This document explains how they work together.
...
- **Mount merging.** When listing a directory, AFS merges entries from the resolved provider with virtual directory entries from child mounts. This means `ls /` shows both files from the root provider and mount point directories.
...
The Shell is AFS's primary interaction interface. It provides a familiar command-line environment with:
...
v.Mount("/data", localfs.New("/workspace"))
```

## Concurrency Model

AFS is designed for parallel-safe use:
