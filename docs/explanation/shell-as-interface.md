...
Most agent frameworks interact with tools through structured API calls — JSON-schema-defined functions that the LLM invokes by name. AFS takes a different approach: **the shell is the interface.**
...
With AFS shell, the same task is one call:
...
## AFS Shell vs. Real Shell

AFS shell is not bash. It deliberately limits scope:
...
From the agent framework's perspective, AFS exposes a single tool:

```json
{
  "name": "shell",
  "description": "Execute a command in the Agent File System",
  "parameters": {
...
A key advantage of the filesystem metaphor is **discoverability**. An agent dropped into an unfamiliar AFS environment can orient itself:
